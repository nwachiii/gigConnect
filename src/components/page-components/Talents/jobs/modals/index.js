...
export const OfferDeclined = ({offerDeclineModal}) => {
	const router = useRouter();
	return (
		<Modal isOpen={offerDeclineModal.isOpen} onClose={offerDeclineModal.onClose}>
			<ModalOverlay bg='rgba(0,0,0,0.1)' />
			<ModalContent rounded={'16px'} pt={'34px'} pb={'42px'} maxW={'315px'} w={'full'}>
				<VStack align={'center'} gap={2}>
					<Image src='/img/icons/check-failure.gif' boxSize={'68px'} objectFit={'cover'} />
					<Heading maxW={'225px'} lineHeight={'32px'} fontSize={20} textAlign={'center'}>
					...
					<Button p={'6px 14px'} rounded={'12px'} border='1px solid #EDEEEF' bg={'#F6F7F7'} fontWeight={400} h='32px' color='#4C5361' w={'max-content'}>
						Message Client
					</Button>
				</VStack>
			</ModalContent>
