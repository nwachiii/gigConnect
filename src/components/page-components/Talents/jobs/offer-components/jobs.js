		...
		<Stack>
			{receivedOffers.length > 0 && receivedOffers ? (
				receivedOffers.map((job, index) => {
					return <JobListing link={`/my-jobs/offers/${job.id}`} handleJob={handleSaveJob} job={job} key={index} buttonText={'View Offer'} />;
				})
			) : (
				<OfferEmptyState headerText={'Received Offers'} desc={'received'} image={'/img/icons/receivedJobsEmpty.svg'} />
			)}
		</Stack>
	);
};
	...
	return (
		<Stack>
			{activeJobs?.length > 0 && activeJobs ? (
				activeJobs.map((job, index) => {
					return (
						<JobListing
							...
							key={index}
							component={
								<Button bg={'#F6F7F7'} color={'#4C5361'} rounded={'12px'} fontWeight={400} p={'6px 16px'} w={'max-content'} fontSize={14} h={'32px'}>
									Message
								</Button>
							}
				...
				<OfferEmptyState headerText={'Active Jobs'} desc={'actively'} image={'/img/icons/activeJobEmpty.svg'} />
			)}
		</Stack>
	);
};
	...
	return (
		<Stack>
			{completedJobs?.length > 0 && completedJobs ? (
				...
				<OfferEmptyState headerText={'Completed Jobs'} desc={'completed'} image={'/img/icons/completedJobsEmpty.svg'} />
			)}
		</Stack>
	);
};
