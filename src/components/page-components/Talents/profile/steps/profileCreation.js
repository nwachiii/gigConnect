...
import {Box, Button, Flex, HStack, Heading, Input, InputGroup, InputLeftElement, Stack, Text, VStack, useToast} from '@chakra-ui/react';
import {useMemo, useState} from 'react';
import {HiMiniXMark} from 'react-icons/hi2';
import {RiSearchLine} from 'react-icons/ri';
import {IoIosAdd} from 'react-icons/io';
import {FormikProvider, useFormik} from 'formik';
import { skillSchema } from '@/components/page-components/profile-components/lib/schema';
...
export const ProfileSkillsStep = ({setStep}) => {
	...
	const filteredSkills = useMemo(() => listSkills.filter((skill) => skill.toLowerCase().includes(searchTerm.toLowerCase()) && !formik.values.skills.includes(skill)), [listSkills, searchTerm, formik.values.skills]);
	...
	return (
		<VStack gap={4} align={'start'} w={'full'} px={6}>
			...
			<VStack align={'start'} w={'full'} maxW={'410px'}>
				...
				<Stack flexWrap={'wrap'} h={formik.values.skills.length > 0 ? 'auto' : '48px'} border={'1px solid #EDEEEF'} rounded={'8px'} p={'8px'} w={'full'} align={formik.values.skills.length > 0 ? 'start' : 'center'} flexDirection={'row'}>
					{formik.values.skills.length > 0 ? (
						formik.values.skills.map((skill, index) => {
							return (
								<Button key={index} bg={'#053AF9'} border={'1px solid #EDEEEF'} padding={'5px 11px'} rounded={'full'} w={'max-content'} gap={'4px'} color={'white'} h={'28px'} fontWeight={500}>
				...
				<VStack w={'full'} align={'start'}>
					...
					<Flex gap={2} pt={2} maxW={'485px'} flexWrap={'wrap'}>
						{filteredSkills.map((skill, index) => {
							return (
								<Button key={index} bg={'#F6F7F7'} border={'1px solid #EDEEEF'} padding={'5px 11px'} rounded={'14px'} w={'max-content'} h={'28px'} onClick={() => addSkill(skill)} gap={'4px'}>
			...
			<HStack gap={2} mb={4}>
				<Button bg={'#F6F7F7'} color={'#4C5361'} rounded={'12px'} fontWeight={400} px={4} py={'6px'} fontSize={14} h={'max-content'} onClick={() => setStep((prev) => prev + 1)} border={'1px solid #EDEEEF'}>
					Skip for now
				</Button>
				<Button bg={'#053AF9'} color={'white'} rounded={'12px'} fontWeight={400} px={4} py={'6px'} fontSize={14} h={'max-content'} onClick={() => setStep((prev) => prev + 1)} isDisabled={!formik.isValid}>
