...
export const experienceSchema = Yup.object().shape({
	experiences: Yup.array().of(experienceValidation).required('At least one experience is required'),
});
...
export const educationSchema = Yup.object().shape({
	education: Yup.array().of(educationValidation).required('At least one education is required'),
});
...
export const skillSchema = Yup.object().shape({
	skills: Yup.array().max(15, 'Max skills reached').min(1, 'At least one skill is required').required(),
});

export const serviceSchema = Yup.object().shape({
	services: Yup.array().max(15, 'Max services reached').min(1, 'At least one service is required').required(),
});

export const languageSchema = Yup.object().shape({
	languages: Yup.array().of(languageValidation).required('At least one language is required'),
});

export const overviewSchema = Yup.object().shape({
	userOverview: Yup.string().min(100, 'The overview cannot be less than 100 characters').required('This field is required'),
});
