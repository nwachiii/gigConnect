		...
		<Stack maxW='1440px' w='full' padding='16px 44px' alignItems='center' justifyContent='space-between' spacing='36px' mx='auto'>
			...
			<Tabs variant='unstyled' w='full' tabIndex={activeIndex}>
				<HStack align='start' w='full' gap={8}>
					<Stack borderRadius='12px' border='1px solid #EDEEEF' w='full' p='8px' align='start' maxW='350px' bg='#FFF' gap={4}>
						{tabs.map((group, groupIndex) => (
							<Stack w='full' gap={1} key={groupIndex}>
								...
								<TabList alignItems='start' flexDirection='column'>
									{group.tabs.map((tab) => (
										<Flex w='full' rounded='12px' bg={activeIndex === tab.index ? '#F6F7F7' : '#FFF'} align='center' p='8px' gap={2} key={tab.index} h='48px'>
											{tab.icon({
					...
					<TabPanels>
						{tabs.map((group) =>
							group.tabs.map((tab, tabIndex) => (
								<TabPanel flex={1} w='full' p={0} key={tabIndex}>
									{tab.component()}
								</TabPanel>
							))
						)}
