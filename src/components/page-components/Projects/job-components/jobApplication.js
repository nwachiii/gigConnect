...
import {Button, Checkbox, Flex, HStack, Heading, Image, Input, Stack, Text, VStack, useDisclosure, useToast} from '@chakra-ui/react';
import {useParams, useRouter} from 'next/navigation';
import {useState} from 'react';
import dynamic from 'next/dynamic';
import {BsChevronLeft} from 'react-icons/bs';
import {ApplicationSuccessful} from '../JobApplicationSuccessModal';
			...
			<Stack borderRadius={'16px'} gap={4} minW='981px' w='full' bg='#FFFFFF'>
				...
				<Stack p={4}>
					...
					<VStack align={'start'} borderRadius='12px' border='1px solid #EDEEEF' w='full' p={4}>
						<Flex direction={'column'} borderRadius='12px' border='1px solid #EDEEEF' w='full' p={4} align={'start'} gap={6}>
							<Text fontWeight={700} fontSize={18}>
								Proposed Contract Terms
							</Text>
							<HStack align={'end'}>
								<Text fontWeight={700}>Your proposed hourly rate for this project</Text>
								<Image src='/img/icons/questionBox.svg' />
							</HStack>
							{isSaved ? (
								<Flex border={'1px solid #EDEEEF'} p={'5px 11px'} h={'max-content'} rounded={'full'} gap={2} bg={'#F6F7F7'}>
									<Text fontWeight={500} color={'#4C5361'}>{`$${parseFloat(hourlyFee).toFixed(2)}/hr`}</Text>
									<Image src='/img/icons/edit-pencil.svg' />
								</Flex>
							) : (
								<HStack align={'start'}>
									<Flex border={'1px solid #EDEEEF'} p={'10px 14px'} h={'44px'} rounded={'8px'}>
										/hr
									</Flex>
									<Input
										value={hourlyFee}
							...
							<Text color={'#707581'} lineHeight={'20px'} maxW={'530px'}>
								A lower hourly rate might give you a greater chance of getting this job. Keep in mind service fees applies
							</Text>
							<Stack>
								<HStack align={'end'}>
						...
						<HStack w={'full'} justify={'end'} gap={2} my={4}>
							<Button bg={'#202329'} rounded={'12px'} fontWeight={400} p={'12px 16px'} h={'max-content'} color={'#fff'} border={'1px solid #EDEEEF'} onClick={() => router.back(-1)}>
								Cancel
							</Button>
							<Button bg={'#053AF9'} rounded={'12px'} fontWeight={400} p={'12px 16px'} h={'max-content'} color={'#fff'} border={'1px solid #EDEEEF'} onClick={modalDisclosure.onOpen} isDisabled={!isSaved || !isChecked}>
								Send Application
							</Button>
						</HStack>
					</VStack>
