import { Menu, MenuButton, MenuList, MenuItem, Avatar, Box, Switch, Flex, Text, VStack, useColorModeValue, Icon } from '@chakra-ui/react';
...
export const UserProfileMenu = () => {
	...
	return (
		<Menu>
			...
			<MenuList bg={useColorModeValue('white', 'gray.700')} borderColor={useColorModeValue('gray.200', 'gray.600')}>
				...
				<VStack divider={<Box borderColor={useColorModeValue('gray.200', 'gray.600')} />} spacing={0}>
					<MenuItem icon={<Icon as={FaUserPlus} />}>Invite a Coworker</MenuItem>
					<MenuItem onClick={() => router.push('/talent/settings')} icon={<Icon as={FaCog} />}>
						Settings
					</MenuItem>
					<MenuItem onClick={() => router.push('/auth/sign-in')} icon={<Icon as={FaSignOutAlt} />}>
						Log out
					</MenuItem>
				</VStack>
			</MenuList>
